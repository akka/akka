...
object ShardRegion {
  ...
  private[akka] class HandOffStopper(shard: String, replyTo: ActorRef, entities: Set[ActorRef], stopMessage: Any, handoffTimeout: FiniteDuration)
    ...
    def receive = {
      case ReceiveTimeout ⇒
        log.warning("HandOffStopMessage[{}] is not handled by some of the entites of the `{}` shard, " +
          "force stopping the remaining entities.", stopMessage.getClass.getName, shard)
