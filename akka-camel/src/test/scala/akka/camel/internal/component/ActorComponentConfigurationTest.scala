...
class ActorComponentConfigurationTest extends JUnitSuite  with ShouldMatchers with SharedCamelSystem{
  ...
  @Test def configurationSanityTest = {
    val actorEndpointConfig = component.createEndpoint("actor://path:akka://test/user/$a?autoack=false&blocking=Blocking%28123000000+nanos%29&outTimeout=987000000+nanos").asInstanceOf[ActorEndpointConfig]
