...
public class ActorCompile {
      ...
      intercept(
          new BehaviorInterceptor<MyMsg, MyMsg>() {
            ...
            public Behavior<MyMsg> aroundReceive(
                ActorContext<MyMsg> ctx, MyMsg msg, ReceiveTarget<MyMsg> target) {
              return target.apply(ctx, msg);
            }

            @Override
            public Behavior<MyMsg> aroundSignal(
                ActorContext<MyMsg> ctx, Signal signal, SignalTarget<MyMsg> target) {
  ...
  Behavior<MyMsg> actor10 =
      Behaviors.receive((ctx, msg) -> stopped(actor4), (ctx, signal) -> same());
  ...
  static class MyBehavior extends ExtensibleBehavior<MyMsg> {
    ...
    public Behavior<MyMsg> receive(ActorContext<MyMsg> ctx, MyMsg msg) throws Exception {
      ActorRef<String> adapter =
          ctx.asJava().messageAdapter(String.class, s -> new MyMsgB(s.toUpperCase()));
      return this;
    }
  }
