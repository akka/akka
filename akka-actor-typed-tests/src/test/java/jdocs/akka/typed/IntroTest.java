...
public class IntroTest {
    ...
    public static Behavior<ChatRoom.SessionEvent> behavior() {
      return Behaviors.receive(ChatRoom.SessionEvent.class)
          ...
          .onMessage(
              ...
              (ctx, msg) -> {
                System.out.println(
                    "message has been posted by '" + msg.screenName + "': " + msg.message);
                return Behaviors.stopped();
              })
          .build();
    }
  ...
  public static void runChatRoom() throws Exception {
    ...
    Behavior<Void> main =
        Behaviors.setup(
            ctx -> {
              ActorRef<ChatRoom.RoomCommand> chatRoom = ctx.spawn(ChatRoom.behavior(), "chatRoom");
              ActorRef<ChatRoom.SessionEvent> gabbler = ctx.spawn(Gabbler.behavior(), "gabbler");
              ctx.watch(gabbler);
