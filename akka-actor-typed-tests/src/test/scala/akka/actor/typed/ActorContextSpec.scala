  ...
  "An ActorContext" must {
    ...
    "correctly wire the lifecycle hook" in {
      ...
      val internal = Behaviors
        .receivePartial[Command] { case (_, Fail) =>
    ...
    "signal post stop after voluntary termination" in {
      ...
      val behavior: Behavior[Command] =
        Behaviors
          .receivePartial[Command] { case (_, Stop) =>
            Behaviors.stopped
          }
          .receiveSignal { case (_, signal) =>
            probe.ref ! ReceivedSignal(signal)
    ...
    "restart and stop a child actor" in {
      ...
      val parent: Behavior[Command] = Behaviors.setup[Command](context => {
        ...
        Behaviors
          .receivePartial[Command] { case (context, StopRef(ref)) =>
            context.stop(ref)
            Behaviors.same
          }
          .receiveSignal { case (_, signal) =>
    ...
    "stop upon stop" in {
      ...
      val watcher: ActorRef[Command] = spawn(
        Behaviors
          .receivePartial[Any] { case (context, Ping) =>
      ...
      spawn(
        Behaviors
          .setup[Command](context => {
            ...
            probe.ref ! ChildMade(childRef)
            Behaviors
              .receivePartial[Command] { case (_, Ping) =>
                probe.ref ! Pong
                Behaviors.same
              }
              .receiveSignal { case (_, signal) =>
      ...
      val actor = spawn(
        Behaviors
          .setup[Command](context => {
            ...
            probe.ref ! ChildMade(childRef)
            Behaviors
              .receivePartial[Command] { case (context, Watch(ref)) =>
                context.watch(ref)
                probe.ref ! Pong
                Behaviors.same
              }
      ...
      val actor = spawn(
        Behaviors
          .setup[Command](context => {
            ...
            probe.ref ! ChildMade(childRef)
            Behaviors
              .receivePartial[Command] { case (_, Inert) =>
                probe.ref ! InertEvent
                Behaviors
                  .receive[Command] { case (_, _) =>
                    Behaviors.unhandled
                  }
                  .receiveSignal {
    ...
    "return the right context info" in {
      ...
      val actor = spawn(
        Behaviors
          .receivePartial[String] { case (context, "info") =>
    ...
    "schedule a message" in {
      ...
      val actor = spawn(
        Behaviors
          .receivePartial[Command] { case (context, Ping) =>
    ...
    "not have problems stopping already stopped child" in {
      ...
      val actor = spawn(Behaviors.setup[Command](context => {
        ...
        Behaviors.receivePartial[Command] { case (context, StopRef(ref)) =>
          context.stop(ref)
          probe.ref ! Pong
          Behaviors.same
        }
