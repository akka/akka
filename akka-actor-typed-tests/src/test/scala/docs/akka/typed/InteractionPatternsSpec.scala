...
import akka.NotUsed
import akka.actor.typed.{ ActorRef, ActorSystem, Behavior, WrappedBehaviorId, TypedAkkaSpecWithShutdown }
...
class InteractionPatternsSpec extends ActorTestKit with TypedAkkaSpecWithShutdown {
  ...
  "contain a sample for ask" in {
    ...
    val hal = spawn(Behaviors.monitor(monitor.ref, halBehavior, new WrappedBehaviorId))
