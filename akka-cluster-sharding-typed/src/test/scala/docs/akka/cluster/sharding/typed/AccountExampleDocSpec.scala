...
class AccountExampleDocSpec
  ...
  "Account" must {
    ...
    "be created with zero balance" in {
      val result = eventSourcedTestKit.runCommand[ReplyWithStatus[Done]](AccountEntity.CreateAccount(_))
      result.reply shouldBe ReplyWithStatus.Ack
      result.event shouldBe AccountEntity.AccountCreated
      result.stateOfType[AccountEntity.OpenedAccount].balance shouldBe 0
    }

    "handle Withdraw" in {
      eventSourcedTestKit.runCommand[ReplyWithStatus[Done]](AccountEntity.CreateAccount(_))

      val result1 = eventSourcedTestKit.runCommand[ReplyWithStatus[Done]](AccountEntity.Deposit(100, _))
      result1.reply shouldBe ReplyWithStatus.Ack
      result1.event shouldBe AccountEntity.Deposited(100)
      result1.stateOfType[AccountEntity.OpenedAccount].balance shouldBe 100

      val result2 = eventSourcedTestKit.runCommand[ReplyWithStatus[Done]](AccountEntity.Withdraw(10, _))
      result2.reply shouldBe ReplyWithStatus.Ack
    ...
    "reject Withdraw overdraft" in {
      eventSourcedTestKit.runCommand[ReplyWithStatus[Done]](AccountEntity.CreateAccount(_))
      eventSourcedTestKit.runCommand[ReplyWithStatus[Done]](AccountEntity.Deposit(100, _))

      val result = eventSourcedTestKit.runCommand[ReplyWithStatus[Done]](AccountEntity.Withdraw(110, _))
