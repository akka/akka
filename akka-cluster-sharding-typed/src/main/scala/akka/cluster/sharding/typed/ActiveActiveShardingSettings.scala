...
object ActiveActiveShardingSettings {
  ...
  def create[M, E](
    ...
    apply[M, E](allReplicaIds.asScala.toVector)((key, replica, _) =>
      settingsPerReplicaFactory(key, replica, allReplicaIds))
  }
  ...
  def apply[M: ClassTag, E](allReplicaIds: immutable.Seq[ReplicaId])(
      settingsPerReplicaFactory: (
          EntityTypeKey[M],
          ReplicaId,
          immutable.Seq[ReplicaId]) => ActiveActiveShardingReplicaSettings[M, E])
...
object ActiveActiveShardingReplicaSettings {
  ...
  /** Java API: */
  def create[M, E](replicaId: ReplicaId, entity: Entity[M, E]): ActiveActiveShardingReplicaSettings[M, E] =
    apply(replicaId, entity)
