...
class FormFieldDirectivesSpec extends RoutingSpec {
  ...
  lazy val multipartForm = MultipartFormData {
    Map(
      "firstName" -> BodyPart("Mike"),
      "age" -> BodyPart(marshalUnsafe(<int>42</int>)),
      "VIPBoolean" -> BodyPart("true"))
  }
  lazy val multipartFormWithTextHtml = MultipartFormData {
    Map(
      "firstName" -> BodyPart("Mike"),
      "age" -> BodyPart(marshalUnsafe(<int>42</int>)),
  ...
  "The 'formFields' extraction directive" should {
    "properly extract the value of www-urlencoded form fields (using the general HList-based variant)" in pendingUntilFixed {
      Get("/", urlEncodedForm) ~> {
    ...
    "properly extract the value of www-urlencoded form fields (using the simple non-HList-based variant)" in pendingUntilFixed {
      Get("/", urlEncodedForm) ~> {
    ...
    "properly extract the value of www-urlencoded form fields when an explicit deserializer is given" in pendingUntilFixed {
      Get("/", urlEncodedForm) ~> {
    ...
    "properly extract the value of multipart form fields (using the general HList-based variant)" in pendingUntilFixed {
      Get("/", multipartForm) ~> {
    ...
    "properly extract the value of multipart form fields (using the simple non-HList-based variant)" in pendingUntilFixed {
      Get("/", multipartForm) ~> {
    ...
    "extract an uploaded FormFile from multipart form fields" in pendingUntilFixed {
    ...
    "reject the request with a MissingFormFieldRejection if a required form field is missing" in pendingUntilFixed {
      Get("/", urlEncodedForm) ~> {
    ...
    "properly extract the value if only a urlencoded deserializer is available for a multipart field that comes without a" +
      "Content-Type (or text/plain)" in pendingUntilFixed {
        Get("/", multipartForm) ~> {
    ...
    "create a proper error message if only a urlencoded deserializer is available for a multipart field with custom " +
      "Content-Type" in pendingUntilFixed {
    ...
    "create a proper error message if only a multipart unmarshaller is available for a www-urlencoded field" in pendingUntilFixed {
  ...
  "The 'formField' requirement directive" should {
    "block requests that do not contain the required formField" in pendingUntilFixed {
      Get("/", urlEncodedForm) ~> {
