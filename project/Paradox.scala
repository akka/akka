...
object Paradox {
  private val MajorMinorRegexp = """(\d+\.\d+).*""".r

  val propertiesSettings = Seq(Compile / paradoxProperties ++= {
    val akkaMajorMinorVersion = {
      version.value match {
        case MajorMinorRegexp(minor) => minor
        case surprisingVersion       => surprisingVersion
      }
    }
    Map(
