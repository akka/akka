...
import akka.http.server.japi.impl.{ ExtractionImplBase, ExtractionImpl, RouteStructure }
...
abstract class HttpBasicAuthenticator[T](val realm: String) extends AbstractDirective with ExtractionImplBase[T] with RequestVal[T] {
  protected[japi] implicit def classTag: ClassTag[T] = reflect.classTag[AnyRef].asInstanceOf[ClassTag[T]]
   ...
   * INTERNAL API
   */
  protected[japi] final def createRoute(first: Route, others: Array[Route]): Route =
