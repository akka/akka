...
import akka.http.model.japi.{ MediaType, ContentType }
import akka.http.server
import akka.http.server.japi.Route
import akka.http.server.japi.impl.RouteStructure._
...
object FileAndResourceRoute {
   ...
   * INTERNAL API
   */
  private[japi] def apply(f: ContentTypeResolver ⇒ Route): FileAndResourceRoute =
   ...
   * INTERNAL API
   */
  private[japi] def forFixedName(fileName: String)(f: ContentType ⇒ Route): FileAndResourceRoute =
