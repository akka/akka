...
@FunctionalInterface
trait CommandToEffect[Command, Event, State] {
  /**
...
final class CommandHandlerBuilder[Command, Event, State] @InternalApi private(rootCommandClass: Class[Command]) {

  private final case class CommandHandlerCase(predicate: Command => Boolean, handler: CommandToEffect[Command, Event, State])

  private var cases: List[CommandHandlerCase] = Nil

  private def addCase(predicate: Command => Boolean, handler: CommandToEffect[Command, Event, State]): Unit = {
    cases = CommandHandlerCase(predicate, handler) :: cases
  }
  /**
    * Match any command which the given `predicate` returns true for
    */
  def matchCommand(predicate: FI.TypedPredicate[Command], commandToEffect: CommandToEffect[Command, Event, State]): CommandHandlerBuilder[Command, Event, State] = {
    addCase(cmd => predicate.defined(cmd), commandToEffect.asInstanceOf[CommandToEffect[Command, Event, State]])
    this
  }

  def build(): CommandHandler[Command, Event, State] = new CommandHandler[Command, Event, State] {
    def apply(ctx: ActorContext[Command], state: State, command: Command) = {
     ???
    }
  }
}
