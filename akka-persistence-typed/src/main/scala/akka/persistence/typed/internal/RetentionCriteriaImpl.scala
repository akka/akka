...
@InternalApi private[akka] final case class SnapshotCountRetentionCriteriaImpl(
  ...
  def deleteUpperSequenceNr(lastSequenceNr: Long): Long = {
    ...
    math.max(0, lastSequenceNr - math.multiplyExact(keepNSnapshots.asInstanceOf[Number].longValue(), snapshotEveryNEvents))
  }
  ...
  def deleteLowerSequenceNr(upperSequenceNr: Long): Long = {
    ...
    math.max(0, upperSequenceNr - math.multiplyExact(keepNSnapshots.asInstanceOf[Number].longValue(), snapshotEveryNEvents))
  }
