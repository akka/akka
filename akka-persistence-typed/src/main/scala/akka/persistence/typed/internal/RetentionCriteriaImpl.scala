...
@InternalApi private[akka] final case class SnapshotCountRetentionCriteriaImpl(
  ...
  def deleteUpperSequenceNr(lastSequenceNr: Long): Long = {
    ...
    math.max(0, lastSequenceNr - (keepNSnapshots.asInstanceOf[Number].longValue() * snapshotEveryNEvents))
  }
  ...
  def deleteLowerSequenceNr(upperSequenceNr: Long): Long = {
    ...
    math.max(0, upperSequenceNr - (keepNSnapshots.asInstanceOf[Number].longValue() * snapshotEveryNEvents))
  }
