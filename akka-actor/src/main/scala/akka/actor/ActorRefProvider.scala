...
class LocalActorRefProvider(
  ...
  lazy val threadContainer = new ThreadPathContainer(system.provider, rootPath / "threads", rootGuardian, log)

  def createThreadActor(queue: collection.mutable.Queue[(Any, ActorRef)]): ActorRef = {
    // no need for unique id as threads ids are unique within one JVM
    val path = rootPath / "threads" / ("thread-" + Thread.currentThread().getId().toString)
