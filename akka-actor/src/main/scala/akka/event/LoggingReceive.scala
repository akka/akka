 ...
 */
class LoggingReceive(source: Option[AnyRef], r: Receive, label: Option[String], logLevel: LogLevel)(implicit
  ...
  def isDefinedAt(o: Any): Boolean = {
    ...
    if (context.system.eventStream.logLevel >= logLevel) {
      ...
      val message =
        "received " + (if (handled) "handled" else "unhandled") + " message " + o + " from " + context.sender() +
        (label match {
