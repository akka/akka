...
private[io] object TcpConnection {
  ...
  object EmptyPendingWrite extends PendingWrite {
    def commander: ActorRef = throw new IllegalStateException("`commander` called on EmptyPendingWrite")
    def doWrite(info: ConnectionInfo): PendingWrite = throw new IllegalStateException("`doWrite` called on EmptyPendingWrite")
    def release(): Unit = throw new IllegalStateException("`release` called on EmptyPendingWrite")
  }
}
