...
/* Note:
 * This is pretty much a straight copy of the Scala 2.13 batching executor, when support for 2.12 is dropped it can be replaced
 * with using the Scala standard library version instead.
 */
 ...
 * INTERNAL API
 */
@InternalApi private[akka] trait Batchable {
  self: Runnable =>
}
 ...
 * INTERNAL API
 */
@InternalApi private[akka] object BatchingExecutorStatics {
  ...
  object MissingParentBlockContext extends BlockContext {
    override def blockOn[T](thunk: => T)(implicit permission: CanAwait): T =
      try thunk
      finally throw new IllegalStateException("BUG in BatchingExecutor.Batch: parentBlockContext is null")
  }

  /*
   * Akka specific: Backport of BlockContext.usingBlockContext from Scala 2.13
   */
  private final val contextLocal = {
    val module = BlockContext.getClass.getDeclaredField("MODULE$").get(BlockContext)
    val field = module.getClass.getDeclaredField("contextLocal")
    field.setAccessible(true)
    field.get(module).asInstanceOf[ThreadLocal[BlockContext]]
  }

  def usingBlockContext[I, T](blockContext: BlockContext)(f: BlockContext => T): T = {
    BlockContext.withBlockContext(blockContext)(f(blockContext))
  ...
  private[this] final def prefer(candidate: BlockContext): BlockContext =
    if (candidate ne null) candidate
      ...
      else BlockContext.defaultBlockContext
    }
...
private[akka] trait BatchingExecutor extends Executor {
  ...
  private[this] final class AsyncBatch private (_first: Runnable, _other: Array[Runnable], _size: Int)
    ...
    override final def run(): Unit = {
      ...
      val f = resubmit(BatchingExecutorStatics.usingBlockContext(this)(this))
  ...
  private[this] final class SyncBatch(runnable: Runnable)
      extends AbstractBatch(runnable, BatchingExecutorStatics.emptyBatchArray, 1)
      with Runnable {
