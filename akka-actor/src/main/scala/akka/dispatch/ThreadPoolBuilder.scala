...
case class ThreadPoolConfig(allowCorePoolTimeout: Boolean = ThreadPoolConfig.defaultAllowCoreThreadTimeout,
  ...
  import ThreadPoolConfig.{ BlockContextThreadFactory }
  ...
  class ThreadPoolExecutorServiceFactory(val threadFactory: ThreadFactory) extends ExecutorServiceFactory {
    def createExecutorService: ExecutorService = {
      val newThreadFactory: ThreadFactory =
        if (minUnblockedThreads > 0) new BlockContextThreadFactory(threadFactory, minUnblockedThreads)
        else threadFactory
      ...
      newThreadFactory match {
        case bctf: BlockContextThreadFactory ⇒
...
case class MonitorableThreadFactory(name: String,
  ...
  @deprecated("use the copy method instead", "2.3.9")
