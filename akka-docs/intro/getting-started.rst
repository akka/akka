...
- ``akka-actor-1.3-RC1`` -- Standard Actors
- ``akka-typed-actor-1.3-RC1`` -- Typed Actors
- ``akka-remote-1.3-RC1`` -- Remote Actors
- ``akka-stm-1.3-RC1`` -- STM (Software Transactional Memory), transactors and transactional datastructures
- ``akka-http-1.3-RC1`` -- Akka Mist for continuation-based asynchronous HTTP and also Jersey integration
- ``akka-slf4j-1.3-RC1`` -- SLF4J Event Handler Listener
- ``akka-testkit-1.3-RC1`` -- Toolkit for testing Actors

We also have Akka Modules containing add-on modules outside the core of Akka.

- ``akka-kernel-1.3-RC1`` -- Akka microkernel for running a bare-bones mini application server (embeds Jetty etc.)
- ``akka-amqp-1.3-RC1`` -- AMQP integration
- ``akka-camel-1.3-RC1`` -- Apache Camel Actors integration (it's the best way to have your Akka application communicate with the rest of the world)
- ``akka-camel-typed-1.3-RC1`` -- Apache Camel Typed Actors integration
- ``akka-scalaz-1.3-RC1`` -- Support for the Scalaz library
- ``akka-spring-1.3-RC1`` -- Spring framework integration
- ``akka-osgi-dependencies-bundle-1.3-RC1`` -- OSGi support
...
2) Add the Akka dependencies. For example, here is the dependency for Akka Actor 1.3-RC1:
