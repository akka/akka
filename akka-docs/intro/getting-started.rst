...
- ``akka-actor-2.0-RC1.jar`` -- Standard Actors, Typed Actors and much more
- ``akka-remote-2.0-RC1.jar`` -- Remote Actors
- ``akka-slf4j-2.0-RC1.jar`` -- SLF4J Event Handler Listener
- ``akka-testkit-2.0-RC1.jar`` -- Toolkit for testing Actors
- ``akka-kernel-2.0-RC1.jar`` -- Akka microkernel for running a bare-bones mini application server
- ``akka-<storage-system>-mailbox-2.0-RC1.jar`` -- Akka durable mailboxes
...
2) Add the Akka dependencies. For example, here is the dependency for Akka Actor 2.0-RC1:
