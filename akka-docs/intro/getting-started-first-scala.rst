...
Let's get the ``akka-actors-1.3-RC4.zip`` distribution of Akka from
...
``AKKA_HOME`` variable::
    ...
    $ cd /Users/jboner/tools/akka-actors-1.3-RC4
    $ export AKKA_HOME=`pwd`
    $ echo $AKKA_HOME
    /Users/jboner/tools/akka-actors-1.3-RC4
...
``scala-library.jar`` JAR) is the ``akka-actor-1.3-RC4.jar`` JAR in the ``lib/akka``
directory. This is a self-contained JAR with zero dependencies and contains
everything we need to write a system using Actors.

Akka is very modular and has many JARs for containing different features. The core distribution has seven modules:

- ``akka-actor-1.3-RC4.jar`` -- Standard Actors
- ``akka-typed-actor-1.3-RC4.jar`` -- Typed Actors
- ``akka-remote-1.3-RC4.jar`` -- Remote Actors
- ``akka-stm-1.3-RC4.jar`` -- STM (Software Transactional Memory), transactors and transactional datastructures
- ``akka-http-1.3-RC4.jar`` -- Akka Mist for continuation-based asynchronous HTTP and also Jersey integration
- ``akka-slf4j-1.3-RC4.jar`` -- SLF4J Event Handler Listener for logging with SLF4J
- ``akka-testkit-1.3-RC4.jar`` -- Toolkit for testing Actors
...
- ``akka-kernel-1.3-RC4.jar`` -- Akka microkernel for running a bare-bones mini application server (embeds Jetty etc.)
- ``akka-amqp-1.3-RC4.jar`` -- AMQP integration
- ``akka-camel-1.3-RC4.jar`` -- Apache Camel Actors integration (it's the best way to have your Akka application communicate with the rest of the world)
- ``akka-camel-typed-1.3-RC4.jar`` -- Apache Camel Typed Actors integration
- ``akka-scalaz-1.3-RC4.jar`` -- Support for the Scalaz library
- ``akka-spring-1.3-RC4.jar`` -- Spring framework integration
- ``akka-osgi-dependencies-bundle-1.3-RC4.jar`` -- OSGi support
...
SBT itself needs a whole bunch of dependencies but our project will only need one; ``akka-actor-1.3-RC4.jar``. SBT will download that as well.
...
First we need to compile the source file. That is done with Scala's compiler ``scalac``. Our application depends on the ``akka-actor-1.3-RC4.jar`` JAR file, so let's add that to the compiler classpath when we compile the source::

    $ scalac -cp lib/akka/akka-actor-1.3-RC4.jar tutorial/Pi.scala

When we have compiled the source file we are ready to run the application. This is done with ``java`` but yet again we need to add the ``akka-actor-1.3-RC4.jar`` JAR file to the classpath, and this time we also need to add the Scala runtime library ``scala-library.jar`` and the classes we compiled ourselves::

    $ java \
        -cp lib/scala-library.jar:lib/akka/akka-actor-1.3-RC4.jar:. \
        akka.tutorial.first.scala.Pi
    AKKA_HOME is defined as [/Users/jboner/tools/akka-actors-1.3-RC4]
    loading config from [/Users/jboner/tools/akka-actors-1.3-RC4/config/akka.conf].
