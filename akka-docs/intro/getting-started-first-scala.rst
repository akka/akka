...
On Windows systems::
    ...
    C:\Users\jboner\src\akka\akka-2.0.2-SNAPSHOT> set PATH=%PATH%;scala-2.9.1\bin
...
On Windows systems::
    ...
    C:\Users\jboner\src\akka\akka-2.0.2-SNAPSHOT> scala -version
    Scala code runner version 2.9.1.final -- Copyright 2002-2011, LAMP/EPFL
...
example adding ``akka-remote``::
    ...
    libraryDependencies += "com.typesafe.akka" % "akka-remote" % "2.0.2-SNAPSHOT"

So, now we are all set.

SBT itself needs a whole bunch of dependencies but our project will only need
one; ``akka-actor-2.0.2-SNAPSHOT.jar``. SBT will download that as well.
...
``scalac``. Our application depends on the ``akka-actor-2.0.2-SNAPSHOT.jar`` JAR
file, so let's add that to the compiler classpath when we compile the source.

On Linux/Unix/Mac systems::

    $ scalac -cp lib/akka/akka-actor-2.0.2-SNAPSHOT.jar Pi.scala

On Windows systems::

    C:\Users\jboner\src\akka\akka-2.0.2-SNAPSHOT> scalac -cp lib\akka\akka-actor-2.0.2-SNAPSHOT.jar Pi.scala
...
``akka-actor-2.0.2-SNAPSHOT.jar`` JAR file to the classpath, and this time we also
...
On Windows systems::
    ...
    C:\Users\jboner\src\akka\akka-2.0.2-SNAPSHOT> sbt
    > compile
    ...
