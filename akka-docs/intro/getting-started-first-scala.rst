...
Let's get the ``akka-2.0.2-RC1.zip`` distribution of Akka from
...
On Linux/Unix/Mac systems::
    ...
    $ cd /Users/jboner/tools/akka-2.0.2-RC1
    $ export AKKA_HOME=`pwd`
    $ echo $AKKA_HOME
    /Users/jboner/tools/akka-2.0.2-RC1

On Windows systems::

    C:\Users\jboner\src\akka> cd akka-2.0.2-RC1
    C:\Users\jboner\src\akka\akka-2.0.2-RC1> set AKKA_HOME=%cd%
    C:\Users\jboner\src\akka\akka-2.0.2-RC1> echo %AKKA_HOME%
    C:\Users\jboner\src\akka\akka-2.0.2-RC1
...
On Windows systems::
    ...
    C:\Users\jboner\src\akka\akka-2.0.2-RC1> dir
    bin
    config
    deploy
    doc
...
``scala-library.jar`` JAR) is the ``akka-actor-2.0.2-RC1.jar`` JAR in the ``lib/akka``
...
.. - ``akka-stm-2.0.2-RC1.jar`` -- STM (Software Transactional Memory), transactors and transactional datastructures
.. - ``akka-camel-2.0.2-RC1.jar`` -- Apache Camel Actors integration (it's the best way to have your Akka application communicate with the rest of the world)
.. - ``akka-camel-typed-2.0.2-RC1.jar`` -- Apache Camel Typed Actors integration
.. - ``akka-spring-2.0.2-RC1.jar`` -- Spring framework integration
...
On Windows systems::
    ...
    C:\Users\jboner\src\akka\akka-2.0.2-RC1> set PATH=%PATH%;scala-2.9.1\bin
...
On Windows systems::
    ...
    C:\Users\jboner\src\akka\akka-2.0.2-RC1> scala -version
    Scala code runner version 2.9.1.final -- Copyright 2002-2011, LAMP/EPFL
...
in the directory you want to create your project in::
    ...
    libraryDependencies += "com.typesafe.akka" % "akka-actor" % "2.0.2-RC1"
...
example adding ``akka-remote``::
    ...
    libraryDependencies += "com.typesafe.akka" % "akka-remote" % "2.0.2-RC1"

So, now we are all set.

SBT itself needs a whole bunch of dependencies but our project will only need
one; ``akka-actor-2.0.2-RC1.jar``. SBT will download that as well.
...
``scalac``. Our application depends on the ``akka-actor-2.0.2-RC1.jar`` JAR
file, so let's add that to the compiler classpath when we compile the source.

On Linux/Unix/Mac systems::

    $ scalac -cp lib/akka/akka-actor-2.0.2-RC1.jar Pi.scala

On Windows systems::

    C:\Users\jboner\src\akka\akka-2.0.2-RC1> scalac -cp lib\akka\akka-actor-2.0.2-RC1.jar Pi.scala
...
``akka-actor-2.0.2-RC1.jar`` JAR file to the classpath, and this time we also
...
On Linux/Unix/Mac systems::
    ...
    $ java \
        -cp lib/scala-library.jar:lib/akka/akka-actor-2.0.2-RC1.jar:. \
...
On Windows systems::
    ...
    C:\Users\jboner\src\akka\akka-2.0.2-RC1> java \
        -cp lib/scala-library.jar;lib\akka\akka-actor-2.0.2-RC1.jar;. \
        akka.tutorial.first.scala.Pi
...
On Windows systems::
    ...
    C:\Users\jboner\src\akka\akka-2.0.2-RC1> sbt
    > compile
    ...
