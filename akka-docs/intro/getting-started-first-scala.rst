...
Let's get the ``akka-2.0-RC3.zip`` distribution of Akka from
...
``AKKA_HOME`` variable::
    ...
    $ cd /Users/jboner/tools/akka-2.0-RC3
    $ export AKKA_HOME=`pwd`
    $ echo $AKKA_HOME
    /Users/jboner/tools/akka-2.0-RC3
...
``scala-library.jar`` JAR) is the ``akka-actor-2.0-RC3.jar`` JAR in the ``lib/akka``
...
.. - ``akka-stm-2.0-RC3.jar`` -- STM (Software Transactional Memory), transactors and transactional datastructures
.. - ``akka-camel-2.0-RC3.jar`` -- Apache Camel Actors integration (it's the best way to have your Akka application communicate with the rest of the world)
.. - ``akka-camel-typed-2.0-RC3.jar`` -- Apache Camel Typed Actors integration
.. - ``akka-spring-2.0-RC3.jar`` -- Spring framework integration
...
example adding ``akka-remote``::
    ...
    libraryDependencies += "com.typesafe.akka" % "akka-remote" % "2.0-RC3"

So, now we are all set.

SBT itself needs a whole bunch of dependencies but our project will only need
one; ``akka-actor-2.0-RC3.jar``. SBT will download that as well.
...
``scalac``. Our application depends on the ``akka-actor-2.0-RC3.jar`` JAR
file, so let's add that to the compiler classpath when we compile the source::

    $ scalac -cp lib/akka/akka-actor-2.0-RC3.jar Pi.scala
...
``akka-actor-2.0-RC3.jar`` JAR file to the classpath, and this time we also
need to add the Scala runtime library ``scala-library.jar`` and the classes we
compiled ourselves::

    $ java \
        -cp lib/scala-library.jar:lib/akka/akka-actor-2.0-RC3.jar:. \
