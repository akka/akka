...
Let's get the ``akka-2.0.2.zip`` distribution of Akka from
...
``scala-library.jar`` JAR) is the ``akka-actor-2.0.2.jar`` JAR in the ``lib/akka``
...
.. - ``akka-stm-2.0.2.jar`` -- STM (Software Transactional Memory), transactors and transactional datastructures
.. - ``akka-camel-2.0.2.jar`` -- Apache Camel Actors integration (it's the best way to have your Akka application communicate with the rest of the world)
.. - ``akka-camel-typed-2.0.2.jar`` -- Apache Camel Typed Actors integration
.. - ``akka-spring-2.0.2.jar`` -- Spring framework integration
...
On Windows systems::
    ...
    C:\Users\jboner\src\akka\akka-2.0.2> set PATH=%PATH%;scala-2.9.1\bin
...
On Windows systems::
    ...
    C:\Users\jboner\src\akka\akka-2.0.2> scala -version
    Scala code runner version 2.9.1.final -- Copyright 2002-2011, LAMP/EPFL
...
one; ``akka-actor-2.0.2.jar``. SBT will download that as well.
...
``scalac``. Our application depends on the ``akka-actor-2.0.2.jar`` JAR
...
``akka-actor-2.0.2.jar`` JAR file to the classpath, and this time we also
...
On Windows systems::
    ...
    C:\Users\jboner\src\akka\akka-2.0.2> sbt
    > compile
    ...
