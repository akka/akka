...
one; ``akka-actor-2.0-RC1.jar``. SBT will download that as well.
...
``scalac``. Our application depends on the ``akka-actor-2.0-RC1.jar`` JAR
file, so let's add that to the compiler classpath when we compile the source::

    $ scalac -cp lib/akka/akka-actor-2.0-RC1.jar Pi.scala
...
``akka-actor-2.0-RC1.jar`` JAR file to the classpath, and this time we also
