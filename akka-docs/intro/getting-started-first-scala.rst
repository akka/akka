...
Let's get the ``akka-actors-1.1.4-SNAPSHOT.zip`` distribution of Akka from
...
``AKKA_HOME`` variable::
    ...
    $ cd /Users/jboner/tools/akka-actors-1.1.4-SNAPSHOT
    $ export AKKA_HOME=`pwd`
    $ echo $AKKA_HOME
    /Users/jboner/tools/akka-actors-1.1.4-SNAPSHOT
...
``scala-library.jar`` JAR) is the ``akka-actor-1.1.4-SNAPSHOT.jar`` JAR in the ``lib/akka``
...
- ``akka-slf4j-1.1.4-SNAPSHOT.jar`` -- SLF4J Event Handler Listener for logging with SLF4J
...
SBT itself needs a whole bunch of dependencies but our project will only need one; ``akka-actor-1.1.4-SNAPSHOT.jar``. SBT downloads that as well.
...
First we need to compile the source file. That is done with Scala's compiler ``scalac``. Our application depends on the ``akka-actor-1.1.4-SNAPSHOT.jar`` JAR file, so let's add that to the compiler classpath when we compile the source::

    $ scalac -cp lib/akka/akka-actor-1.1.4-SNAPSHOT.jar tutorial/Pi.scala

When we have compiled the source file we are ready to run the application. This is done with ``java`` but yet again we need to add the ``akka-actor-1.1.4-SNAPSHOT.jar`` JAR file to the classpath, and this time we also need to add the Scala runtime library ``scala-library.jar`` and the classes we compiled ourselves::

    $ java \
        -cp lib/scala-library.jar:lib/akka/akka-actor-1.1.4-SNAPSHOT.jar:. \
        akka.tutorial.first.scala.Pi
    AKKA_HOME is defined as [/Users/jboner/tools/akka-actors-1.1.4-SNAPSHOT]
    loading config from [/Users/jboner/tools/akka-actors-1.1.4-SNAPSHOT/config/akka.conf].
