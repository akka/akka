...
Let's get the ``akka-2.0-RC2.zip`` distribution of Akka from
...
``AKKA_HOME`` variable::
    ...
    $ cd /Users/jboner/tools/akka-2.0-RC2
    $ export AKKA_HOME=`pwd`
    $ echo $AKKA_HOME
    /Users/jboner/tools/akka-2.0-RC2
...
``scala-library.jar`` JAR) is the ``akka-actor-2.0-RC2.jar`` JAR in the ``lib/akka``
...
.. - ``akka-stm-2.0-RC2.jar`` -- STM (Software Transactional Memory), transactors and transactional datastructures
.. - ``akka-camel-2.0-RC2.jar`` -- Apache Camel Actors integration (it's the best way to have your Akka application communicate with the rest of the world)
.. - ``akka-camel-typed-2.0-RC2.jar`` -- Apache Camel Typed Actors integration
.. - ``akka-spring-2.0-RC2.jar`` -- Spring framework integration
...
``javac``. Our application depends on the ``akka-actor-2.0-RC2.jar`` and the
``scala-library.jar`` JAR files, so let's add them to the compiler classpath
when we compile the source::

    $ javac -cp lib/scala-library.jar:lib/akka/akka-actor-2.0-RC2.jar tutorial/akka/tutorial/first/java/Pi.java

When we have compiled the source file we are ready to run the application. This
is done with ``java`` but yet again we need to add the ``akka-actor-2.0-RC2.jar``
and the ``scala-library.jar`` JAR files to the classpath as well as the classes
we compiled ourselves::

    $ java \
        -cp lib/scala-library.jar:lib/akka/akka-actor-2.0-RC2.jar:. \
