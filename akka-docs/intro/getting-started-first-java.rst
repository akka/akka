...
Let's get the ``akka-actors-1.3.zip`` distribution of Akka from
...
``AKKA_HOME`` variable::
    ...
    $ cd /Users/jboner/tools/akka-actors-1.3
    $ export AKKA_HOME=`pwd`
    $ echo $AKKA_HOME
    /Users/jboner/tools/akka-actors-1.3
...
``scala-library.jar`` JAR) is the ``akka-actor-1.3.jar`` JAR in the ``lib/akka``
...
- ``akka-slf4j-1.3.jar`` -- SLF4J Event Handler Listener for logging with SLF4J
...
``javac``. Our application depends on the ``akka-actor-1.3.jar`` and the
``scala-library.jar`` JAR files, so let's add them to the compiler classpath
when we compile the source::

    $ javac -cp lib/scala-library.jar:lib/akka/akka-actor-1.3.jar tutorial/akka/tutorial/first/java/Pi.java

When we have compiled the source file we are ready to run the application. This
is done with ``java`` but yet again we need to add the ``akka-actor-1.3.jar``
and the ``scala-library.jar`` JAR files to the classpath as well as the classes
we compiled ourselves::

    $ java \
        -cp lib/scala-library.jar:lib/akka/akka-actor-1.3.jar:tutorial \
        akka.tutorial.java.first.Pi
    AKKA_HOME is defined as [/Users/jboner/tools/akka-actors-1.3]
    loading config from [/Users/jboner/tools/akka-actors-1.3/config/akka.conf].
