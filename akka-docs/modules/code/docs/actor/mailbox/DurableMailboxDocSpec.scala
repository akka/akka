...
class MyMessageQueue(_owner: ActorContext)
  ...
  def enqueue(receiver: ActorRef, envelope: Envelope): Unit = withSyncCircuitBreaker {
    val data: Array[Byte] = serialize(envelope)
    storage.push(data)
  }

  def dequeue(): Envelope  = withSyncCircuitBreaker {
    val data: Option[Array[Byte]] = storage.pull()
    data.map(deserialize).orNull
  }

  def hasMessages: Boolean = withSyncCircuitBreaker { !storage.isEmpty }

  def numberOfMessages: Int =  withSyncCircuitBreaker { storage.size }
