...
object TestKitDocSpec {
  ...
  class TestTimerActor extends Actor with Timers {
    override def receive = { case TriggerScheduling(foo) =>
      triggerScheduling(ScheduledMessage(foo))
    }
  ...
  class LoggingActor extends Actor {
    ...
    def otherState: Receive = LoggingReceive.withLabel("other") { case msg => // Do something else ...
    }
    // #logging-receive
  }
}
