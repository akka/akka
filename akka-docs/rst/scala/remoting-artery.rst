...
* Support for a separate "lane" for large messages to avoid interference with smaller messages
...
* Serviceability through the built-in Flight-Recorder
...
an actor has a globally unique reference on a given network.

In cases, where Network Address Translation (NAT) is used or other network bridging is involved, it is important
to configure the system so that it understands that there is a different between his externally visible, canonical
