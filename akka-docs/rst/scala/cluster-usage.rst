...
If you do not have configured seed nodes, then you need to join manually, using :ref:`cluster_jmx_scala`
or :ref:`cluster_command_line_scala`. You can join to any node in the cluster. It doesn't 
have to be configured as a seed node.

Instead of joining manually, joining can also be performed programmatically with ``Cluster(system).join``. Note that
you can only join to an existing cluster member, which means that for bootstrapping some
node must join itself.

You may also use ``Cluster(system).joinSeedNodes`` to join programmatically, which is attractive when dynamically 
