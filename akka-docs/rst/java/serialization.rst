...
If using the Akka Protobuf serializers (by enabling ``enable-additional-serialization-bindings``) for the internal
Akka messages those will not require the same major Scala version however you must also ensure the serializers used
for your own types does not introduce the same incompatibility as Java serialization does.
