...
class FlowGroupedWeightedSpec
  ...
  "A GroupedWeighted" must {
    ...
    "fail during stream initialization when minWeight is negative" in {
      val ex =
        the[IllegalArgumentException] thrownBy Source(1 to 5).groupedWeighted(-1)(_ => 1L).to(Sink.collection).run()
    ...
    "fail during stream initialization when minWeight is 0" in {
      val ex =
        the[IllegalArgumentException] thrownBy Source(1 to 5).groupedWeighted(0)(_ => 1L).to(Sink.collection).run()
      ex.getMessage should be("requirement failed: minWeight must be greater than 0")
    }
