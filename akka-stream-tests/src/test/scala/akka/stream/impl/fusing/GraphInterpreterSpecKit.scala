...
trait GraphInterpreterSpecKit extends AkkaSpec {
  ...
  abstract class OneBoundedSetup[T](ops: Array[GraphStageWithMaterializedValue[_, _]]) extends Builder {
    ...
    def this(ops: Iterable[Stage[_, _]]) = {
      this(ops.map { op ⇒
        new PushPullGraphStage[Any, Any, Any](
          ...
          Attributes.none)
      }.toArray.asInstanceOf[Array[GraphStageWithMaterializedValue[_, _]]])
    }
    ...
    private def initialize(): Unit = {
      ...
      manualInit(GraphAssembly(ops, attributes, ins, inOwners, outs, outOwners))
