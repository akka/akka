...
public class OutputStreamSourceTest extends StreamTest {
    ...
    public void mustSendEventsViaOutputStream() throws Exception {
        ...
        final Source<ByteString, Pair<OutputStream, Future<Long>>> source = OutputStreamSource.create(timeout);
        final Pair<OutputStream, Future<Long>> p = source.to(Sink.foreach(new Procedure<ByteString>() {
            public void apply(ByteString elem) {
                probe.getRef().tell(elem, ActorRef.noSender());
            }
        })).run(materializer);

        p.first().write("a".getBytes());
