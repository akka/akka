...
public class FlowTest extends StreamTest {
  ...
  public void mustBeAbleToUseMapAsyncForFutureWithNullResult() throws Exception {
      ...
      Flow<Integer, Void, NotUsed> flow = Flow.of(Integer.class).mapAsync(1, x -> CompletableFuture.completedFuture(null));
      List<Void> result = Source.from(input).via(flow).runWith(Sink.seq(), materializer).toCompletableFuture().get(3, TimeUnit.SECONDS);
