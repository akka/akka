...
public class BidiFlowTest extends StreamTest {
  ...
  public void mustWorkAsAFlowThatIsOpenOnTheLeft() throws Exception {
    ...
    final Future<List<String>> result = Source.from(list).via(f).limit(10).runWith(Sink.<String>seq(), materializer);
  ...
  public void mustWorkAsAFlowThatIsOpenOnTheRight() throws Exception {
    ...
    final Future<List<Long>> result = Source.from(inputs).via(f).limit(10).runWith(Sink.<Long>seq(), materializer);
