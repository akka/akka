...
import akka.actor._
import akka.stream.{ AbruptTerminationException, ActorFlowMaterializerSettings, Attributes, ActorMaterializer }
...
private[akka] object ActorInterpreter {
  def props(settings: ActorFlowMaterializerSettings, ops: Seq[Stage[_, _]], materializer: ActorMaterializer, attributes: Attributes = Attributes.none): Props =
 ...
 * INTERNAL API
 */
private[akka] class ActorInterpreter(val settings: ActorFlowMaterializerSettings, val ops: Seq[Stage[_, _]], val materializer: ActorMaterializer, val attributes: Attributes)
