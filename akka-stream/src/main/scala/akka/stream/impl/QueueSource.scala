        ...
        if (!buffer.isFull) {
          ...
          overflowStrategy match {
            ...
            case s: DropBuffer =>
              ...
              enqueueAndSuccess(offer)
            case s: DropNew =>
              log.log(s.logLevel, "[{}] Dropping the new element because buffer is full and overflowStrategy is: [DropNew]", name)
            ...
            case s: Backpressure =>
              log.log(s.logLevel, "[{}] Backpressuring because buffer is full and overflowStrategy is: [Backpressure]", name)
              pendingOffer match {
                case Some(_) =>
          ...
          if (maxBuffer != 0) {
            ...
            overflowStrategy match {
              case s @ (_: DropHead | _: DropBuffer) =>
                log.log(s.logLevel, "[{}] Dropping element because buffer is full and overflowStrategy is: [{}]", name, s)
              ...
              case s: Fail =>
                ...
                failStage(bufferOverflowException)
              case s: Backpressure =>
                log.log(s.logLevel, "[{}] Failing because buffer is full and overflowStrategy is: [Backpressure]", name)
                promise.failure(
                  new IllegalStateException(
