...
object RestartSink {
  ...
  def withBackoff[T](minBackoff: FiniteDuration, maxBackoff: FiniteDuration, randomFactor: Double)(
    ...
    Sink.fromGraph(new RestartWithBackoffSink(sinkFactory, minBackoff, maxBackoff, randomFactor, maxRestarts = Int.MaxValue, lifetimeMaxRestarts = Int.MaxValue))
  }
   ...
   * backoff.
   *
   * This [[Sink]] will not cancel as long as maxRestarts is not reached (TODO clarify), since cancellation by the wrapped [[Sink]]
  ...
  def withBackoff[T](minBackoff: FiniteDuration, maxBackoff: FiniteDuration, randomFactor: Double, maxRestarts: Int)(
      sinkFactory: () => Sink[T, _]): Sink[T, NotUsed] = {
    Sink.fromGraph(new RestartWithBackoffSink(sinkFactory, minBackoff, maxBackoff, randomFactor, maxRestarts, lifetimeMaxRestarts = Int.MaxValue))
  }
   ...
   * backoff.
   *
   * This [[Sink]] will not cancel as long as maxRestarts or lifetimeMaxRestarts is not reached (TODO clarify), since cancellation by the wrapped [[Sink]]
   * is handled by restarting it. The wrapped [[Sink]] can however be completed by feeding a completion or error into
   * this [[Sink]]. When that happens, the [[Sink]], if currently running, will terminate and will not be restarted.
   * This can be triggered simply by the upstream completing, or externally by introducing a [[KillSwitch]] right
   * before this [[Sink]] in the graph.
  ...
  def withBackoff[T](
      ...
      randomFactor: Double,
      maxRestarts: Int,
      lifetimeMaxRestarts: Int)(sinkFactory: () => Sink[T, _]): Sink[T, NotUsed] = {
    Sink.fromGraph(
      new RestartWithBackoffSink(sinkFactory, minBackoff, maxBackoff, randomFactor, maxRestarts, lifetimeMaxRestarts))
  }
