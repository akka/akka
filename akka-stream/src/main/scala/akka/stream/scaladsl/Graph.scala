...
final class Partition[T](val outputPorts: Int, val partitioner: T => Int, val eagerCancel: Boolean)
  ...
  val out: Seq[Outlet[T]] = Seq.tabulate(outputPorts)(i =>
    Outlet[T]("Partition.out" + i)) // FIXME BC make this immutable.IndexedSeq as type + Vector as concrete impl
  ...
  override def createLogic(inheritedAttributes: Attributes): GraphStageLogic =
    new GraphStageLogic(shape) with InHandler {
      ...
      out.iterator.zipWithIndex.foreach { case (o, idx) =>
        setHandler(
          o,
          new OutHandler {
            override def onPull() = {
...
class ZipWithN[A, O](zipper: immutable.Seq[A] => O)(n: Int) extends GraphStage[UniformFanInShape[A, O]] {
  ...
  override def createLogic(inheritedAttributes: Attributes): GraphStageLogic =
    new GraphStageLogic(shape) with OutHandler {
      ...
      shape.inlets.zipWithIndex.foreach { case (in, i) =>
        setHandler(
          in,
          new InHandler {
            override def onPush(): Unit = {
...
object GraphDSL extends GraphApply {
  ...
  object Implicits {
    ...
    implicit final class BidiFlowShapeArrow[I1, O1, I2, O2](val bidi: BidiShape[I1, O1, I2, O2]) extends AnyVal {
      ...
      def <~>[I3, O3, M](otherFlow: Graph[BidiShape[O1, O3, I3, I2], M])(implicit
