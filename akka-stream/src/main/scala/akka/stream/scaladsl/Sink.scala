...
object Sink {
   ...
   */
  @deprecated("Use variant accepting both on complete and on failure message", "2.6.0")
  def actorRefWithAck[T](ref: ActorRef, onInitMessage: Any, ackMessage: Any, onCompleteMessage: Any): Sink[T, NotUsed] =
    actorRefWithAck(ref, _ => identity, _ => onInitMessage, ackMessage, onCompleteMessage, Status.Failure)
