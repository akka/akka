...
class DuctSpec extends AkkaSpec {
  ...
  "A Duct" must {
    ...
    "materialize into Producer/Consumer and multiple transformation processors" in {
      val duct: Duct[Int, String] = Duct[Int].map((i: Int) ⇒ i.toString).map("elem-" + _)
      val (ductIn: Consumer[Int], ductOut: Producer[String]) = duct.build(materializer)
    ...
    "perform transformation operation" in {
      val duct: Duct[Int, String] = Duct[Int].map((i: Int) ⇒ { testActor ! i.toString; i.toString })
      val c: Consumer[Int] = duct.consume(materializer)
    ...
    "perform multiple transformation operations" in {
      val duct: Duct[Int, Unit] = Duct[Int].map(_.toString).map("elem-" + _).foreach(testActor ! _)
      val c: Consumer[Int] = duct.consume(materializer)
