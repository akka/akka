...
class FlowIteratorSpec extends AkkaSpec {
  ...
  "A Flow based on an iterator" must {
    ...
    "produce elements with one transformation step" in {
      val p = Flow(List(1, 2, 3).iterator).map(_ * 2).toPublisher(materializer)
      val c = StreamTestKit.SubscriberProbe[Int]()
      p.subscribe(c)
      val sub = c.expectSubscription()
      sub.request(10)
    ...
    "produce elements with two transformation steps" in {
      val p = Flow(List(1, 2, 3, 4).iterator).filter(_ % 2 == 0).map(_ * 2).toPublisher(materializer)
      val c = StreamTestKit.SubscriberProbe[Int]()
      p.subscribe(c)
      val sub = c.expectSubscription()
      sub.request(10)
    ...
    "allow cancel before receiving all elements" in {
      val count = 100000
      val p = Flow((1 to count).iterator).toPublisher(materializer)
      val c = StreamTestKit.SubscriberProbe[Int]()
      p.subscribe(c)
      val sub = c.expectSubscription()
      sub.request(count)
