  ...
  "Timed" must {
    ...
    "measure time it takes to go through intermediate" in {
      ...
      val printInfo = (interval: Duration) ⇒ {
        ...
        info(s"Measured interval between $measureBetweenEvery elements was: ${interval.pretty(includeNanos = true)}")
      }
    ...
    "measure time it takes from start to complete, by wrapping operations" in {
      ...
      val n = 500
      val printInfo = (d: Duration) ⇒ {
        testActor.ref ! d
        info(s"Processing $n elements took ${d.pretty(includeNanos = true)}")
      }
