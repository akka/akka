...
public class ReactiveStreamsDocTest {
  ...
  final Fixture.RS rs = new Fixture.RS() {
    ...
    public Publisher<Tweet> tweets() {
      return TwitterStreamQuickstartDocTest.Model.tweets.runWith(Sink.publisher(1), mat);
    }
    ...
    new JavaTestKit(system) {
      ...
      
      {
        ...
        final Publisher<Author> authorPublisher =
          Source.from(rs.tweets()).via(authors).runWith(Sink.publisher(1), mat);
    ...
    new JavaTestKit(system) {
      ...
      
      {
        ...
        final Publisher<Author> authorPublisher =
          Source.from(rs.tweets())
            .via(authors)
            .runWith(Sink.publisher(Int.MaxValue), mat);
