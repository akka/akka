  ...
  public void chunker() throws Exception {
    new JavaTestKit(system) {
...

      {
        ...
        Future<List<ByteString>> chunksFuture = chunksStream.limit(10).runWith(Sink.seq(), mat);
  ...
  public void limiterShouldWork() throws Exception {
    new JavaTestKit(system) {
...

      {
        ...
        List<ByteString> got = Await.result(bytes1.via(limiter).limit(10).runWith(Sink.seq(), mat), threeSeconds);
        ...
        try {
          Await.result(bytes2.via(limiter).limit(10).runWith(Sink.seq(), mat), threeSeconds);
    ...
    new JavaTestKit(system) {
      {
        ...
        List<ByteString> got = Await.result(compacted.limit(10).runWith(Sink.seq(), mat), timeout);
