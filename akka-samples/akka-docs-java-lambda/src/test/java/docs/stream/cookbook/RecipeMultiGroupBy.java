  ...
  public void work() throws Exception {
    new JavaTestKit(system) {
...

      {
        ...
        final Source<Message> elems = Source
          .from(Arrays.asList("1: a", "1: b", "all: c", "all: d", "1: e"))
          .map(s -> new Message(s));

        //#multi-groupby
        final Source<Pair<Message, Topic>> messageAndTopic = elems.mapConcat((Message msg) -> {
        ...
        Source<Pair<Topic, Source<Message>>> multiGroups =
          messageAndTopic
            .groupBy(pair -> pair.second())
            .map(pair -> {
              Topic topic = pair.first();
              Source<Pair<Message, Topic>> topicStream = pair.second();

              // chopping of the topic from the (Message, Topic) pairs
              return new Pair<Topic, Source<Message>>(topic, topicStream.<Message>map(p -> p.first()));
            });
        //#multi-groupby
        ...
        Future<List<String>> result = multiGroups
          .map(pair -> {
            Topic topic = pair.first();
            Source<String> topicMessages = pair.second().map(p -> p.msg);
