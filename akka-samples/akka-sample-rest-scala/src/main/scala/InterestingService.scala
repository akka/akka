  ...
  def receive =
  {
    ...
    case post:Post =>
    {
      ...
      if (post.request.getContentType startsWith MediaType.APPLICATION_FORM_URLENCODED)
      {
        ...
        if (name.length % 2 == 0)
        {
          post.response.getWriter.write("<p>Action A verified request.</p>")
        }
        else
        {
          post.response.getWriter.write("<p>Action A could not verify request.</p>")
        }
...
class ActionCompleteActor extends Actor
{
  ...
  def complete(req:RequestMethod) =
  {
    requests.get(req.hashCode) match
    {
