...
object Result {
  ...
  def requestMore(n: Int): Result[Nothing] =
    if (n > 0) RequestMore(n) else Continue
}
...
case class Subscribe[T, U](producer: Source[T])(val handlerFactory: SubscriptionResults ⇒ SubscriptionHandler[T, U]) extends ForwardResult[U]
