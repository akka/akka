...
class SpanImplSpec extends FreeSpec with ShouldMatchers with SyncOperationSpec {
  "SpanImpl should" - {
    ...
    "return singleton source when first element of span matches" in pending
    "upstream completes while nothing is requested" in pending
    "upstream completes while waiting for first element" in pending
    "upstream completes while waiting for sub-subscription" in pending
  }
}
