...
class BackoffOnRestartSupervisorSpec extends AkkaSpec with ImplicitSender {
  ...
  def supervisorProps(probeRef: ActorRef) = {
    val options = Backoff.onFailure(TestActor.props(probeRef), "someChildName", 200 millis, 10 seconds, 0.0, -1)
  ...
  "BackoffOnRestartSupervisor" must {
    ...
    "accept commands while child is terminating" in {
      ...
      val options = Backoff.onFailure(Props(new SlowlyFailingActor(postStopLatch)), "someChildName", 1 nanos, 1 nanos, 0.0, -1)
    ...
    "respect withinTimeRange property of OneForOneStrategy" in {
      ...
      // that is acceptable.
      val options = Backoff.onFailure(TestActor.props(probe.ref), "someChildName", 300 millis, 10 seconds, 0.0, -1)
