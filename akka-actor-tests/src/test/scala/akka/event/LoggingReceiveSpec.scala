  ...
  "A LoggingReceive" must {
    ...
    "decorate a Receive" in {
      new TestKit(appLogging) {
        ...
        a ! "hallo"
        expectMsg(1 second, Logging.Debug("funky", classOf[DummyClassForStringSources], "received unhandled message hallo from " + system.deadLetters.toString))
    ...
    "be added on Actor if requested" in {
      new TestKit(appLogging) with ImplicitSender {
        ...
        actor ! "buh"
        within(1 second) {
          expectMsg(Logging.Debug(actor.path.toString, actor.underlyingActor.getClass, "received handled message buh from " + self.toString))
          expectMsg("x")
        }
