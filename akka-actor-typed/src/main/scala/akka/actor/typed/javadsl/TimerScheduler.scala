...
trait TimerScheduler[T] {
   ...
   */
  def withKeyType[K1]: TimerScheduler.KeyTyped[K1, T] = this.asInstanceOf

}

object TimerScheduler {
  // A concrete type because you can't use a Scala type alias as a type in Java
  trait KeyTyped[K1, T] extends TimerScheduler[T] { type K >: K1 }
}
