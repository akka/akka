 ...
 *
 * You can use the factory methods directly, or import `ActorSink.Implicits`
 * to make these methods available on [[akka.stream.scaladsl.Sink]].
 */
object ActorSink {
  import akka.typed.scaladsl.adapter._

  /** Provides typed actor integration methods as extendsion methods on [[akka.stream.scaladsl.Sink]] */
  object Implicits {

    implicit final class TypedEnrichedSink(val s: Sink.type) extends AnyVal {
      def actorRef[T](ref: ActorRef[T], onCompleteMessage: T, onFailureMessage: Throwable ⇒ T): Sink[T, NotUsed] =
        ActorSink.actorRef(ref, onCompleteMessage, onFailureMessage)
      ...
      def actorRefWithAck[T](
        ...
        ActorSink.actorRefWithAck(ref, onInitMessage, ackMessage, onCompleteMessage, onFailureMessage)
    }
