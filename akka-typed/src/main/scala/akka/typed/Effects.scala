...
class EffectfulActorContext[T](_name: String, _initialBehavior: Behavior[T], _mailboxCapacity: Int, _system: ActorSystem[Nothing])
  ...
  def run(msg: T): Unit = current = Behavior.canonicalize(current.receiveMessage(this, msg), current)
  def signal(signal: Signal): Unit = current = Behavior.canonicalize(current.receiveSignal(this, signal), current)
