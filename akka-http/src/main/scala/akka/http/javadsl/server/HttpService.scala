...
trait HttpServiceBase {
   ...
   * Starts a server on the given interface and port and uses the route to handle incoming requests.
   */
  def bindRoute(interface: String, port: Int, route: Route, system: ActorSystem, flowMaterializer: Materializer): Future[ServerBinding] =
   ...
   */
  def handleConnectionsWithRoute(interface: String, port: Int, route: Route, system: ActorSystem, flowMaterializer: Materializer): Future[ServerBinding] = {
    implicit val sys = system
