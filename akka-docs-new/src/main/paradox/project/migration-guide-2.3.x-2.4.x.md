...
migrating @ref[1.3.x to 2.0.x](migration-guide-1.3.x-2.0.x.md#migration-2-0) and then @ref[2.0.x to 2.1.x](migration-guide-2.0.x-2.1.x.md#migration-2-1)
and then @ref[2.1.x to 2.2.x](migration-guide-2.1.x-2.2.x.md#migration-2-2) and then @ref[2.2.x to 2.3.x](migration-guide-2.2.x-2.3.x.md#migration-2-3).
...
the @ref[Akka Typed](../scala/typed.md#typed-scala) project that is currently being developed in open preview mode. If you are using TypedActors
...
Both the `EventStream` (@ref[Scala](../scala/event-bus.md#event-stream-scala), @ref[Java](../java/event-bus.md#event-stream-java)) and the
`ManagedActorClassification`, `ManagedActorEventBus` (@ref[Scala](../scala/event-bus.md#actor-classification-scala), @ref[Java](../java/event-bus.md#actor-classification-java)) now
...
@@snip [EventBusDocSpec.scala](../scala/code/docs/event/EventBusDocSpec.scala) { #actor-bus }

If you have been creating EventStreams manually, you now have to provide an actor system and *start the unsubscriber*:

@@snip [EventStreamSpec.scala](../../../../../akka-actor-tests/src/test/scala/akka/event/EventStreamSpec.scala) { #event-bus-start-unsubscriber-scala }
...
user defined main class and packaging with [sbt-native-packager ](https://github.com/sbt/sbt-native-packager)
or [Lightbend ConductR ](http://www.lightbend.com/products/conductr).
Please see @ref[Use-case and Deployment Scenarios](../intro/deployment-scenarios.md#deployment-scenarios) for more information.
...
Please see @ref[Scala](../scala/cluster-metrics.md#cluster-metrics-scala), @ref[Java](../java/cluster-metrics.md#cluster-metrics-java) for more information.
...
You can use the @ref[RemoveInternalClusterShardingData](../scala/cluster-sharding.md#removeinternalclustershardingdata-scala)
...
@ref[Failures](../scala/persistence.md#failures-scala). 
...
`PersistentView` is deprecated. Use @ref[Persistence Query](../scala/persistence-query.md#persistence-query-scala) instead. The corresponding
 ...
 * [Sink.actorRef](http://doc.akka.io/docs/akka-stream-and-http-experimental/1.0/scala/stream-integrations.html#Sink_actorRef) is simple, but has the disadvantage that there is no back-pressure signal from the 
destination actor, i.e. if the actor is not consuming the messages fast enough the mailbox of the actor will grow
 * [mapAsync](http://doc.akka.io/docs/akka-stream-and-http-experimental/1.0/stages-overview.html#Asynchronous_processing_stages) combined with @ref[Ask: Send-And-Receive-Future](../java/lambda-actors.md#actors-ask-lambda) is almost as simple with the advantage of back-pressure
being propagated all the way
 * [ActorSubscriber](http://doc.akka.io/docs/akka-stream-and-http-experimental/1.0/scala/stream-integrations.html#ActorSubscriber) in case you need more fine grained control
...
See @ref[Journal plugin API for Scala](../scala/persistence.md#journal-plugin-api)
or @ref[Journal plugin API for Java](../java/persistence.md#journal-plugin-api-java).
...
Read the @ref[API documentation](../scala/persistence.md#journal-plugin-api) of this method for more
