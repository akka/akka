...
it might be easier to run them on a @ref[Cluster Singleton](cluster-singleton.md#cluster-singleton-scala) node.
...
tutorial named [Akka Cluster Sharding with Scala! ](http://www.lightbend.com/activator/template/akka-cluster-sharding-scala).
...
persistent (durable), e.g. with @ref[Persistence](persistence.md#persistence-scala), so that it can be recovered at the new
location.
...
@ref[Persistence](persistence.md#persistence-scala) to survive failures. Since it is running in a cluster @ref[Persistence](persistence.md#persistence-scala)
...
at-least-once semantics can be added by using `AtLeastOnceDelivery`  in @ref[Persistence](persistence.md#persistence-scala).
...
Instead of using @ref[Persistence](persistence.md#persistence-scala) it is possible to use the @ref[distributed_data_scala](distributed-data.md#distributed-data-scala) module
as storage for the state of the sharding coordinator. In such case the state of the 
`ShardCoordinator` will be replicated inside a cluster by the @ref[distributed_data_scala](distributed-data.md#distributed-data-scala) module with
...
e.g. with @ref[Persistence](persistence.md#persistence-scala).
...
@@snip [ClusterShardingGracefulShutdownSpec.scala](../../../../../akka-cluster-sharding/src/multi-jvm/scala/akka/cluster/sharding/ClusterShardingGracefulShutdownSpec.scala) { #graceful-shutdown }
