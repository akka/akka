...
making sure to use an async logging backend though. (See [Using the SLF4J API directly](#slf4j-directly-scala))
...
It has one single dependency; the slf4j-api jar. In runtime you also need a SLF4J backend, we recommend [Logback ](http://logback.qos.ch/):
