...
familiarize yourself with @ref[Actor Systems](../general/actor-systems.md#actor-systems) and <!-- FIXME: More than one link target with name supervision in path Some(/scala/actors.rst) --> supervision and it
...
deprecated. On the other hand using this variant in a `Props` factory in
...
Remote actor addresses may also be looked up, if @ref[remoting](remoting.md#remoting-scala) is enabled:
...
See @ref[Futures](futures.md#futures-scala) for more information on how to await or query a
future.
...
mailbox, see the documentation on mailboxes: @ref[Mailboxes](mailboxes.md#mailboxes-scala).
