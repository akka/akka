...
transformation combinators to choose from, see also @ref[stages-overview_scala](stages-overview.md#stages-overview-scala).
...
the `Sink` [companion object ](http://doc.akka.io/api/akka-stream-and-http-experimental/@version@/#akka.stream.scaladsl.Sink$).
...
explained in detail in @ref[Modularity, Composition and Hierarchy](stream-composition.md#composition-scala). It is also possible to wrap complex computation graphs
