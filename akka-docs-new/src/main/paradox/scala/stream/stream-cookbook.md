...
If you need a quick reference of the available processing stages used in the recipes see @ref[stages-overview_scala](stages-overview.md#stages-overview-scala).

## Working with Flows

In this collection we show simple recipes that involve linear flows. The recipes in this section are rather
general, more targeted recipes are available as separate sections (@ref[Buffers and working with rate](stream-rate.md#stream-rate-scala), @ref[Working with streaming IO](stream-io.md#stream-io-scala)).
