...
@@snip [PersistentActorExample.scala](../../../../../akka-samples/akka-sample-persistence-scala/src/main/scala/sample/persistence/PersistentActorExample.scala) { #persistent-actor-example }
...
> **Warning:**
If you are using @ref[Persistence Query](persistence-query.md#persistence-query-scala), query results may be missing deleted messages in a journal,
...
<a id="persistent-views"></a>
## Persistent Views

> **Warning:**
`PersistentView` is deprecated. Use @ref[Persistence Query](persistence-query.md#persistence-query-scala) instead. The corresponding
...
@@snip [PersistenceDocSpec.scala](code/docs/persistence/PersistenceDocSpec.scala) { #view }
...
@@snip [PersistenceDocSpec.scala](code/docs/persistence/PersistenceDocSpec.scala) { #auto-update-interval }
...
@@snip [PersistenceDocSpec.scala](code/docs/persistence/PersistenceDocSpec.scala) { #view-update }
...
@@snip [PersistenceDocSpec.scala](code/docs/persistence/PersistenceDocSpec.scala) { #auto-update }
...
Further possibilities to customize initial recovery are explained in section [Recovery](#recovery-scala).

<a id="persistence-identifiers"></a>
### Identifiers
...
The `viewId` must differ from the referenced `persistenceId`, unless [Snapshots](#snapshots) of a view and its
...
Note that @ref[cluster_sharding_scala](cluster-sharding.md#cluster-sharding-scala) is using snapshots, so if you use Cluster Sharding you need to define a snapshot store plugin.
...
> **Note:**
For more advanced schema evolution techniques refer to the @ref[Persistence - Schema Evolution](persistence-schema-evolution.md#persistence-schema-evolution-scala) documentation.
...
`PersistencePluginProxyExtension` @ref[extension](extending-akka.md#extending-akka-scala), or by calling the `PersistencePluginProxy.start` method.
...
@ref[Serialization](serialization.md#serialization-scala) infrastructure. For example, if an application wants to serialize
