...
and @ref[AtLeastOnceDelivery](persistence.md#at-least-once-delivery-scala) provided by @ref[Akka Persistence](persistence.md#persistence-scala).
...
The block given to `within` must complete after a @ref[Duration](../common/duration.md#duration) which
...
His full example is also available @ref[here](testkit-example.md#testkit-example).
 ...
 * 
   *Logging of message invocations on certain actors*
   This is enabled by a setting in the [Configuration](#configuration) � namely
...
If the aforementioned setting is not given in the [Configuration](#configuration), this method will
