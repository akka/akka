...
old remoting implementation. For the current stable remoting system please refer to @ref[Remoting](remoting.md#remoting-java).
...
@@snip [RemoteDeploymentDocTest.java](code/docs/remoting/RemoteDeploymentDocTest.java) { #make-address-artery }
...
In short, Actor message delivery is “at-most-once” as described in @ref[Message Delivery Reliability](../general/message-delivery-reliability.md#message-delivery-reliability)
 ...
 * Remote failure detector triggers, i.e. remote watch is used. This is different when @ref[Akka Cluster](cluster-usage.md#cluster-usage-java)
...
@ref[Lifecycle Monitoring aka DeathWatch](untyped-actors.md#deathwatch-java). However, it is important to note, that unlike in the local case, remoting has to handle
...
remained the same, we recommend reading the @ref[Serialization](serialization.md#serialization-java) documentation first.

Implementing an `akka.serialization.ByteBufferSerializer` works the same way as any other serializer,

@@snip [ByteBufferSerializerDocTest.java](code/docs/actor/ByteBufferSerializerDocTest.java) { #ByteBufferSerializer-interface }

Implementing a serializer for Artery is therefore as simple as implementing this interface, and binding the serializer 
as usual (which is explained in @ref[Serialization](serialization.md#serialization-java)).
...
@@snip [ByteBufferSerializerDocTest.java](code/docs/actor/ByteBufferSerializerDocTest.java) { #bytebufserializer-with-manifest }
...
remote messages. Please refer to the @ref[Serialization](serialization.md#serialization-java) documentation as well as [ByteBuffer based serialization](#remote-bytebuffer-serialization-java) to learn how to do this.
...
@@snip [RemoteDeploymentDocTest.java](../java/code/docs/remoting/RemoteDeploymentDocTest.java) { #programmatic-artery }
