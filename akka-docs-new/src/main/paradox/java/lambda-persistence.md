<a id="persistence-lambda-java"></a>
...
Akka persistence is inspired by the [eventsourced](https://github.com/eligosource/eventsourced) library. It follows the same concepts and architecture of
[eventsourced](https://github.com/eligosource/eventsourced) but significantly differs on API and implementation level.
...
development of event sourced applications (see section [Event sourcing](#event-sourcing-java-lambda))

<a id="event-sourcing-java-lambda"></a>
## Event sourcing
...
@@snip [PersistentActorExample.java](../../../../../akka-samples/akka-sample-persistence-java-lambda/src/main/java/sample/persistence/PersistentActorExample.java) { #persistent-actor-example }
...
calls in context of a single command. Incoming messages are [stashed](#internal-stash-lambda) until the `persist`
is completed.
...
@@snip [LambdaPersistenceDocTest.java](code/docs/persistence/LambdaPersistenceDocTest.java) { #persistence-id-override }

<a id="recovery-java-lambda"></a>
### Recovery
...
@@snip [LambdaPersistenceDocTest.java](code/docs/persistence/LambdaPersistenceDocTest.java) { #recovery-custom }

Recovery can be disabled by returning `Recovery.none` in the `recovery` method of a `PersistentActor`:

@@snip [LambdaPersistenceDocTest.java](code/docs/persistence/LambdaPersistenceDocTest.java) { #recovery-disabled }

#### Recovery status

A persistent actor can query its own recovery status via the methods

@@snip [LambdaPersistenceDocTest.java](code/docs/persistence/LambdaPersistenceDocTest.java) { #recovery-status }
...
@@snip [LambdaPersistenceDocTest.java](code/docs/persistence/LambdaPersistenceDocTest.java) { #recovery-completed }
...
<a id="internal-stash-lambda"></a>
### Internal stash

The persistent actor has a private @ref[stash](lambda-actors.md#stash-lambda) for internally caching incoming messages during 
[recovery](#recovery-java-lambda) or the `persist\persistAll` method persisting events. You can still 
...
@@snip [LambdaPersistenceDocTest.java](code/docs/persistence/LambdaPersistenceDocTest.java) { #persist-async }
...
@@snip [LambdaPersistenceDocTest.java](code/docs/persistence/LambdaPersistenceDocTest.java) { #defer }
...
@@snip [LambdaPersistenceDocTest.java](code/docs/persistence/LambdaPersistenceDocTest.java) { #defer-caller }

> **Warning:**
The callback will not be invoked if the actor is restarted (or stopped) in between the call to
`deferAsync` and the journal has processed and confirmed all preceding writes.

<a id="nested-persist-calls-lambda"></a>
...
@@snip [LambdaPersistenceDocTest.java](code/docs/persistence/LambdaPersistenceDocTest.java) { #nested-persist-persist }

When sending two commands to this `PersistentActor`, the persist handlers will be executed in the following order:

@@snip [LambdaPersistenceDocTest.java](code/docs/persistence/LambdaPersistenceDocTest.java) { #nested-persist-persist-caller }
...
@@snip [LambdaPersistenceDocTest.java](code/docs/persistence/LambdaPersistenceDocTest.java) { #nested-persistAsync-persistAsync }

In this case no stashing is happening, yet the events are still persisted and callbacks executed in the expected order:

@@snip [LambdaPersistenceDocTest.java](code/docs/persistence/LambdaPersistenceDocTest.java) { #nested-persistAsync-persistAsync-caller }
...
@@snip [LambdaPersistenceDocTest.java](code/docs/persistence/LambdaPersistenceDocTest.java) { #backoff }
...
would restart and try to persist their events again. Instead, using a `BackoffSupervisor` (as described in [Failures](#failures-lambda)) which
...
@@snip [LambdaPersistenceDocTest.java](code/docs/persistence/LambdaPersistenceDocTest.java) { #safe-shutdown }

@@snip [LambdaPersistenceDocTest.java](code/docs/persistence/LambdaPersistenceDocTest.java) { #safe-shutdown-example-bad }

@@snip [LambdaPersistenceDocTest.java](code/docs/persistence/LambdaPersistenceDocTest.java) { #safe-shutdown-example-good }

<a id="persistent-views-java-lambda"></a>
## Persistent Views

> **Warning:**
`AbstractPersistentView` is deprecated. Use @ref[Persistence Query](persistence-query.md#persistence-query-java) instead. The corresponding
...
and setting the �initial behavior� in the constructor by calling the `receive` method.

@@snip [LambdaPersistenceDocTest.java](code/docs/persistence/LambdaPersistenceDocTest.java) { #view }
...
@@snip [LambdaPersistenceDocTest.java](code/docs/persistence/LambdaPersistenceDocTest.java) { #view-update }
...
Further possibilities to customize initial recovery are explained in section [Recovery](#recovery-java-lambda).

<a id="persistence-identifiers-java-lambda"></a>
### Identifiers
...
The `viewId` must differ from the referenced `persistenceId`, unless [Snapshots](#snapshots-java-lambda) of a view and its
persistent actor should be shared (which is what applications usually do not want).

<a id="snapshots-java-lambda"></a>
## Snapshots
...
@@snip [LambdaPersistenceDocTest.java](code/docs/persistence/LambdaPersistenceDocTest.java) { #save-snapshot }

During recovery, the persistent actor is offered a previously saved snapshot via a `SnapshotOffer` message from
which it can initialize internal state.

@@snip [LambdaPersistenceDocTest.java](code/docs/persistence/LambdaPersistenceDocTest.java) { #snapshot-offer }
...
@@snip [LambdaPersistenceDocTest.java](code/docs/persistence/LambdaPersistenceDocTest.java) { #snapshot-criteria }
...
<a id="at-least-once-delivery-java-lambda"></a>
...
@@snip [LambdaPersistenceDocTest.java](code/docs/persistence/LambdaPersistenceDocTest.java) { #at-least-once-example }
...
<a id="persistent-fsm-java-lambda"></a>
## Persistent FSM
...
For an example of journal plugin which writes messages to LevelDB see [Local LevelDB journal](#local-leveldb-journal-java-lambda).
For an example of snapshot store plugin which writes snapshots as individual files to the local filesystem see [Local snapshot store](#local-snapshot-store-java-lambda).
...
@@snip [LambdaPersistencePluginDocTest.java](code/docs/persistence/LambdaPersistencePluginDocTest.java) { #plugin-imports }
...
@@snip [LambdaPersistencePluginDocTest.java](code/docs/persistence/LambdaPersistencePluginDocTest.java) { #sync-journal-plugin-api }
...
<a id="local-leveldb-journal-java-lambda"></a>
...
<a id="shared-leveldb-journal-java-lambda"></a>
...
> **Note:**
This plugin has been supplanted by [Persistence Plugin Proxy](#persistence-plugin-proxy-lambda).
...
<a id="local-snapshot-store-java-lambda"></a>
...
<a id="persistence-plugin-proxy-lambda"></a>
...
<a id="custom-serialization-lambda"></a>
...
> **Warning:**
It is not possible to test persistence provided classes (i.e. @ref[PersistentActor](persistence.md#event-sourcing-java)
and @ref[AtLeastOnceDelivery](persistence.md#at-least-once-delivery-java)) using `TestActorRef` due to its *synchronous* nature.
