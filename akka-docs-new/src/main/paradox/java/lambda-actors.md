...
maintenance releases does not apply to the `akka.actor.AbstractActor`, related classes and
the `akka.japi.pf` package.
...
familiarize yourself with @ref[Actor Systems](../general/actor-systems.md#actor-systems) and <!-- FIXME: More than one link target with name supervision in path Some(/java/lambda-actors.rst) --> supervision and it
...
@@snip [MyActor.java](code/docs/actorlambda/MyActor.java) { #imports #my-actor }
...
@@snip [GraduallyBuiltActor.java](code/docs/actorlambda/GraduallyBuiltActor.java) { #imports #actor }
...
@@snip [ActorDocTest.java](code/docs/actorlambda/ActorDocTest.java) { #import-props }

@@snip [ActorDocTest.java](code/docs/actorlambda/ActorDocTest.java) { #creating-props }
...
@@snip [ActorDocTest.java](code/docs/actorlambda/ActorDocTest.java) { #creating-props-deprecated }
...
@@snip [ActorDocTest.java](code/docs/actorlambda/ActorDocTest.java) { #props-factory }
...
@@snip [ActorDocTest.java](code/docs/actorlambda/ActorDocTest.java) { #messages-in-companion }
...
@@snip [ActorDocTest.java](code/docs/actorlambda/ActorDocTest.java) { #import-actorRef }

@@snip [ActorDocTest.java](code/docs/actorlambda/ActorDocTest.java) { #system-actorOf }
...
@@snip [ActorDocTest.java](code/docs/actorlambda/ActorDocTest.java) { #context-actorOf }
...
<a id="actor-create-factory-lambda"></a>
 ...
 * `sender()` reference sender Actor of the last received message, typically used as described in [LambdaActor.Reply](#lambdaactor-reply)
 * 
 ...
 * 
    ...
    * hotswap behavior stack as described in [Become/Unbecome](#actor-hotswap-lambda)
...
(see [Identifying Actors via Actor Selection](#actorselection-lambda)). This can also be done with the `resolveOne`
method of the `ActorSelection`, which returns a `Future` of the matching
`ActorRef`.

<a id="deathwatch-lambda"></a>
...
@@snip [ActorDocTest.java](code/docs/actorlambda/ActorDocTest.java) { #watch }
...
<a id="start-hook-lambda"></a>
### Start Hook

Right after starting the actor, its `preStart` method is invoked.

@@snip [ActorDocTest.java](code/docs/actorlambda/ActorDocTest.java) { #preStart }
...
<a id="restart-hook-lambda"></a>
### Restart Hooks

All actors are supervised, i.e. linked to another actor with a fault
handling strategy. Actors may be restarted in case an exception is thrown while
processing a message (see <!-- FIXME: More than one link target with name supervision in path Some(/java/lambda-actors.rst) --> supervision). This restart involves the hooks
mentioned above:
...
@@snip [ActorDocTest.java](code/docs/actorlambda/ActorDocTest.java) { #selection-local }
...
>
 * sending messages using the @ref[At-Least-Once Delivery](lambda-persistence.md#at-least-once-delivery-java-lambda) facility
...
@@snip [ActorDocTest.java](code/docs/actorlambda/ActorDocTest.java) { #selection-wildcard }
...
@@snip [ActorDocTest.java](code/docs/actorlambda/ActorDocTest.java) { #import-identify }

@@snip [ActorDocTest.java](code/docs/actorlambda/ActorDocTest.java) { #identify }
...
@ref[Actors (Java with Lambda Support)](lambda-index-actors.md#actor-java-lambda) for Java compatibility). It is completed with failure
...
@@snip [ActorDocTest.java](code/docs/actorlambda/ActorDocTest.java) { #selection-remote }
...
<a id="actors-tell-sender-lambda"></a>
### Tell: Fire-forget

This is the preferred way of sending messages. No blocking waiting for a
message. This gives the best concurrency and scalability characteristics.

@@snip [ActorDocTest.java](code/docs/actorlambda/ActorDocTest.java) { #tell }

The sender reference is passed along with the message and available within the
receiving actor via its `sender()` method while processing this
message. Inside of an actor it is usually `self()` who shall be the
...
@@snip [ActorDocTest.java](code/docs/actorlambda/ActorDocTest.java) { #reply-exception }
...
@@snip [ActorDocTest.java](code/docs/actorlambda/ActorDocTest.java) { #forward }

## Receive messages

An Actor either has to set its initial receive behavior in the constructor by
calling the `receive` method in the `AbstractActor`:

@@snip [ActorDocTest.java](code/docs/actorlambda/ActorDocTest.java) { #receive-constructor }

or by implementing the `receive` method in the `Actor` interface:

@@snip [ActorDocTest.java](code/docs/actorlambda/ActorDocTest.java) { #receive }

Both the argument to the `AbstractActor` `receive` method and the return
type of the `Actor` `receive` method is a `PartialFunction<Object, BoxedUnit>`
...
@@snip [MyActor.java](code/docs/actorlambda/MyActor.java) { #reply }
...
@@snip [ActorDocTest.java](code/docs/actorlambda/ActorDocTest.java) { #receive-timeout }
...
publishing `Terminated` on the [DeathWatch](#deathwatch-lambda), telling
...
@@snip [ActorDocTest.java](code/docs/actorlambda/ActorDocTest.java) { #postStop }
...
@@snip [ActorDocTest.java](code/docs/actorlambda/ActorDocTest.java) { #gracefulStop-actor }
...
@@snip [ActorDocTest.java](code/docs/actorlambda/ActorDocTest.java) { #hot-swap-actor }
...
@@snip [ActorDocTest.java](code/docs/actorlambda/ActorDocTest.java) { #swapper }
...
@@snip [ActorDocTest.java](code/docs/actorlambda/ActorDocTest.java) { #stash }
...
supervision process is started (see <!-- FIXME: More than one link target with name supervision in path Some(/java/lambda-actors.rst) --> supervision). Depending on the
...
@@snip [InitializationDocTest.java](code/docs/actorlambda/InitializationDocTest.java) { #messageInit }
...
<a id="actor-performance-lambda"></a>
...
@ref[untyped actor](untyped-actors.md#untyped-actors-java) version.