...
it might be easier to run them on a @ref[Cluster Singleton](cluster-singleton.md#cluster-singleton-java) node.
...
persistent (durable), e.g. with @ref[Persistence](persistence.md#persistence-java), so that it can be recovered at the new
location.
...
@ref[Persistence](persistence.md#persistence-java) to survive failures. Since it is running in a cluster @ref[Persistence](persistence.md#persistence-java)
...
at-least-once semantics can be added by using `AtLeastOnceDelivery`  in @ref[Persistence](persistence.md#persistence-java).
...
Instead of using @ref[Persistence](persistence.md#persistence-java) it is possible to use the @ref[distributed_data_java](distributed-data.md#distributed-data-java) module
as storage for the state of the sharding coordinator. In such case the state of the 
`ShardCoordinator` will be replicated inside a cluster by the @ref[distributed_data_java](distributed-data.md#distributed-data-java) module with
...
e.g. with @ref[Persistence](persistence.md#persistence-java).
...
@@snip [ClusterShardingTest.java](../../../../../akka-cluster-sharding/src/test/java/akka/cluster/sharding/ClusterShardingTest.java) { #graceful-shutdown }
