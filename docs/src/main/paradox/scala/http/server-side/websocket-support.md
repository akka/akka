...
@scala[Strict messages are represented with the `Strict` subclass for each kind of message which contains data as a strict, i.e. non-streamed, `ByteString` or `String`.]
@java[If `TextMessage.isStrict` returns true, the complete data is already available and can be accessed with `TextMessage.getStrictText` (analogously for `BinaryMessage`).]
...
first, a handler @scala[`Flow[Message, Message, Any]`]@java[`Flow<Message, Message, ?>`] that will be used to handle WebSocket messages on this connection.
...
A message handler is expected to be implemented as a @scala[`Flow[Message, Message, Any]`]@java[`Flow<Message, Message, ?>`]. For typical request-response
...
which allows to pass an output-generating @scala[`Source[Message, Any]`]@java[`Source<Message, ?>`] and an input-receiving @scala[`Sink[Message, Any]`]@java[`Sink<Message, ?>`] independently.
