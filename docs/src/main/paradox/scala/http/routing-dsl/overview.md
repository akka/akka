...
`HttpRequest` @scala[(maybe with the help of a few extractors in the way of [Unfiltered](http://unfiltered.databinder.net/))] this approach becomes somewhat
...
*route structure* which, at its top-level, can be used to create a handler `Flow` (or, alternatively, an async handler function) that
can be directly supplied to a `bind` call. @scala[The conversion from `Route` to flow can either be invoked explicitly
