...
sealed trait HttpEntity extends jm.HttpEntity {
  ...
  /** Java API */
  def getDataBytes: stream.javadsl.Source[ByteString, _] = stream.javadsl.Source.wrap(dataBytes)
...
object HttpEntity {
  ...
  final case class Chunked(contentType: ContentType, chunks: Source[ChunkStreamPart, Any])
    ...
    /** Java API */
    def getChunks: stream.javadsl.Source[jm.ChunkStreamPart, Any] = stream.javadsl.Source.wrap(chunks)
  }
  object Chunked {
    /**
