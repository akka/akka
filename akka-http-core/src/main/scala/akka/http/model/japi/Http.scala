...
import akka.actor.{ ActorSystem, ExtensionId, ExtendedActorSystem, ExtensionIdProvider }
...
class Http(system: ExtendedActorSystem) extends akka.actor.Extension {
  ...
  private lazy val delegate: HttpExt = akka.http.Http(system)
  ...
  def bindAndHandle(handler: Flow[HttpRequest, HttpResponse, _],
    ...
    delegate.bindAndHandle(handler.asInstanceOf[Flow[model.HttpRequest, model.HttpResponse, _]].asScala,
      interface, port)(fm)
  ...
  def bindAndHandleSync(handler: Function[HttpRequest, HttpResponse],
                        interface: String, port: Int,
                        fm: FlowMaterializer): Future[ServerBinding] =
    delegate.bindAndHandleSync(handler.apply(_).asInstanceOf[model.HttpResponse], interface, port)(fm)
      .map(new ServerBinding(_))(ec)
  ...
  def bindAndHandleSync(handler: Function[HttpRequest, HttpResponse],
                        ...
                        log: LoggingAdapter,
                        fm: FlowMaterializer): Future[ServerBinding] =
    delegate.bindAndHandleSync(handler.apply(_).asInstanceOf[model.HttpResponse],
      interface, port, backlog, immutableSeq(options), settings, log)(fm)
      .map(new ServerBinding(_))(ec)
  ...
  def bindAndHandleAsync(handler: Function[HttpRequest, Future[HttpResponse]],
                         interface: String, port: Int,
                         fm: FlowMaterializer): Future[ServerBinding] =
    delegate.bindAndHandleAsync(handler.apply(_).asInstanceOf[Future[model.HttpResponse]], interface, port)(fm)
      .map(new ServerBinding(_))(ec)
  ...
  def bindAndHandleAsync(handler: Function[HttpRequest, Future[HttpResponse]],
                         ...
                         log: LoggingAdapter,
                         fm: FlowMaterializer): Future[ServerBinding] =
    delegate.bindAndHandleAsync(handler.apply(_).asInstanceOf[Future[model.HttpResponse]],
      interface, port, backlog, immutableSeq(options), settings, log)(fm)
      .map(new ServerBinding(_))(ec)
}
