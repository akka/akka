...
private[http] class UriParser(val input: ParserInput,
  ...
  def userinfo = rule {
    clearSB() ~ zeroOrMore(`userinfo-char` ~ append()| `pct-encoded`) ~ '@' ~ run(_userinfo = sb.toString)
  }
  ...
  def `reg-name` = rule(
    clearSBForDecoding() ~ oneOrMore(`lower-reg-name-char` ~ append() | UPPER_ALPHA ~ appendLowered() | `pct-encoded`) ~
  ...
  def pchar = rule { `path-segment-char` ~ append() | `pct-encoded` }

  def query = {
    def part = rule(
      clearSBForDecoding() ~ oneOrMore('+' ~ append(' ') | `query-char` ~ append() | `pct-encoded`) ~ push(getDecodedString())
        | push(""))
    ...
    if (uriParsingMode == Uri.ParsingMode.RelaxedWithRawQuery) rule {
      clearSB() ~ oneOrMore(`query-char` ~ append()) ~ run(_query = Query.Raw(sb.toString)) | run(_query = Query.Empty)
    } else rule { keyValuePairs ~> (_query = _) }
  }

  def fragment = rule(
    clearSBForDecoding() ~ oneOrMore(`fragment-char` ~ append() | `pct-encoded`) ~ run(_fragment = Some(getDecodedString()))
    | run(_fragment = Some("")))
  ...
  def `absolute-path` = rule {
    clearSB() ~ oneOrMore('/' ~ append('/') ~ segment) ~ savePath()
  }
  ...
  def appendLowered(): Rule0 = rule { run(sb.append(CharUtils.toLowerCase(lastChar))) }

  def savePath() = rule { run(_path = Path(sb.toString, uriParsingCharset)) }

  private[this] var firstPercentIx = -1

  def clearSBForDecoding(): Rule0 = rule { run { sb.setLength(0); firstPercentIx = -1 } }

  def getDecodedString(charset: Charset = uriParsingCharset) =
    if (firstPercentIx >= 0) decode(sb.toString, charset, firstPercentIx)() else sb.toString

  def getDecodedStringAndLowerIfEncoded(charset: Charset = uriParsingCharset) =
