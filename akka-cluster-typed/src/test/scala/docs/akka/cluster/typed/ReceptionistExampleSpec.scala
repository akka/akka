...
object PingPongExample {
  ...
  //#pinger
  def pinger(pingService: ActorRef[Ping]) = Behaviors.onStart[Pong.type] { ctx ⇒
  ...
  //#pinger-guardian
  val guardian: Behavior[Listing[Ping]] = Behaviors.onStart { ctx ⇒
  ...
  val guardianJustPingService: Behavior[Listing[Ping]] = Behaviors.onStart { ctx ⇒
    val ps = ctx.spawnAnonymous(pingService)
    ctx.watch(ps)
    Behaviors.immutablePartial[Listing[Ping]] {
      case (c, Listing(PingServiceKey, listings)) if listings.nonEmpty ⇒
  ...
  val guardianJustPinger: Behavior[Listing[Ping]] = Behaviors.onStart { ctx ⇒
    ctx.system.receptionist ! Receptionist.Subscribe(PingServiceKey, ctx.self)
    Behaviors.immutablePartial[Listing[Ping]] {
      case (c, Listing(PingServiceKey, listings)) if listings.nonEmpty ⇒
        listings.foreach(ps ⇒ ctx.spawnAnonymous(pinger(ps)))
