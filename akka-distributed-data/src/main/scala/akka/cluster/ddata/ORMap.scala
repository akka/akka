...
final class ORMap[A, B <: ReplicatedData] private[akka] (
  ...
  private def dryMergeDelta(thatDelta: ORMap.DeltaOp, withValueDeltas: Boolean = false): ORMap[A, B] = {
    ...
    var mergedValues: Map[A, B] = this.values.filter { case (k, _) ⇒ this.keys.contains(k) }
    // FIXME: refactor with partition later
    var tombstonedVals: Map[A, B] = this.values.filterNot { case (k, _) ⇒ this.keys.contains(k) }
