...
object BidiFlowDocSpec {
  ...
  val codecVerbose = BidiFlow.wrap(FlowGraph.create() { b =>
  ...
  //#framing
  val framing = BidiFlow.wrap(FlowGraph.create() { b =>
    implicit val order = ByteOrder.LITTLE_ENDIAN
  ...
  val chopUp = BidiFlow.wrap(FlowGraph.create() { b =>
    val f = Flow[ByteString].mapConcat(_.map(ByteString(_)))
    BidiShape(b.add(f), b.add(f))
  })

  val accumulate = BidiFlow.wrap(FlowGraph.create() { b =>
