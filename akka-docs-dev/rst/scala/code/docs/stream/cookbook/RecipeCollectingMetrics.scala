...
import akka.stream.{ ActorFlowMaterializerSettings, ActorMaterializer }
...
class RecipeCollectingMetrics extends RecipeSpec {
  import HoldOps._
  implicit val m2 = ActorMaterializer(ActorFlowMaterializerSettings(system).withInputBuffer(1, 1))
