...
private [akka] object CouchDBStorageBackend extends
  ...
  def create() = {
	...
	// 	var m = entries.map(e=>(new String(e._1, ENCODING) -> new String(e._2, ENCODING))).toMap
	// 	val dataJson = JsonSerialization.tojson(m)
	// 	postData(URL + name, dataJson.toString)
	// }
	
	private def storeMap(name: String, entries: Map[String, Any]) ={
    postData(URL + name, JsonSerialization.tojson(entries).toString)
  }	
  ...
  def removeMapStorageFor(name: String, key: Array[Byte]): Unit = {
		getResponseForNameAsMap(name).flatMap(doc=>{
		  		removeMapStorageFor(name)
		    	storeMap(name, doc - new String(key, ENCODING))})	
	}
	
  def getMapStorageEntryFor(name: String, key: Array[Byte]): Option[Array[Byte]] = {
	  getResponseForNameAsMap(name).flatMap(_.get(new String(key, ENCODING))).asInstanceOf[Option[String]]
      .map(_.getBytes)
	}
  ...
  def getMapStorageFor(name: String): List[(Array[Byte], Array[Byte])] = {
	...
	def getMapStorageRangeFor(name: String, start: Option[Array[Byte]], finish: Option[Array[Byte]], count: Int): List[(Array[Byte], Array[Byte])] = {
		...
		val s = start.map(new String(_, ENCODING)).getOrElse(keys.head)
