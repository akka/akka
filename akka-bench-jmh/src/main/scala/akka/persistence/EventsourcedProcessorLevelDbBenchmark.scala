...
class EventsourcedProcessorLevelDbBenchmark {

  val config = PersistenceSpec.config("leveldb", "erformanceSpec")
  ...
  def setup() {
    ...
    persist1CommandProcessor = system.actorOf(Props(classOf[Persist1EventEventsourcedProcessor], data10k.last), "p-1")
    persist1EventProcessor = system.actorOf(Props(classOf[Persist1EventEventsourcedProcessor], data10k.last), "ep-1")
    persistAsync1EventProcessor = system.actorOf(Props(classOf[PersistAsync1EventEventsourcedProcessor], data10k.last), "epa-1")
    persistAsync1QuickReplyEventProcessor = system.actorOf(Props(classOf[PersistAsync1EventQuickReplyEventsourcedProcessor], data10k.last), "epa-2")
  }
...
class Persist1EventEventsourcedProcessor(respondAfter: Int) extends EventsourcedProcessor {
...
class PersistAsync1EventQuickReplyEventsourcedProcessor(respondAfter: Int) extends EventsourcedProcessor {
  override def receiveCommand  = {
    case n: Int  =>
      val e = Evt(n)
