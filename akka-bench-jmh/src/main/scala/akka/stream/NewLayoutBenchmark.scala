...
class NewLayoutBenchmark {
  ...
  def source_and_sink_new_linear(): TraversalBuilder = {
    linearSource.traversal.append(linearSink.traversal, linearSink.shape)
  }
  ...
  def source_flow_and_sink_new_linear(): TraversalBuilder = {
    linearSource.traversal
      .append(linearFlow.traversal, linearFlow.shape)
      .append(linearSink.traversal, linearSink.shape)
  }

  @Benchmark
  def source_flow_and_sink_new_composite(): TraversalBuilder = {
    compositeSource.traversal
      .add(compositeFlow.traversal, compositeFlow.shape)
  ...
  val sourceSinkLinear = linearSource.traversal.append(linearSink.traversal, linearSink.shape)
  val sourceSinkComposite = compositeSource.traversal
    .add(compositeSink.traversal, compositeSink.shape)
    .wire(compositeSource.out, compositeSink.in)
  ...
  val sourceFlowSinkLinear = linearSource.traversal
    .append(linearFlow.traversal, linearFlow.shape)
    .append(linearSink.traversal, linearSink.shape)
  val sourceFlowSinkComposite = compositeSource.traversal
    .add(compositeFlow.traversal, compositeFlow.shape)
    .wire(compositeSource.out, compositeFlow.in)
    .add(compositeSink.traversal, compositeSink.shape)
    .wire(compositeFlow.out, compositeSink.in)
