...
class MiscMessageSerializerSpec extends AkkaSpec(MiscMessageSerializerSpec.testConfig) {
  ...
  "MiscMessageSerializer" must {
    Seq(
      ...
      "BroadcastGroup" → BroadcastGroup(List("/user/a", "/user/b")),
      "BroadcastGroup with custom dispatcher" → BroadcastGroup(List("/user/a", "/user/b"), routerDispatcher = "my-dispatcher"),
      ...
      "RandomGroup" → RandomGroup(List("/user/a", "/user/b")),
      "RandomGroup with custom dispatcher" → RandomGroup(List("/user/a", "/user/b"), routerDispatcher = "my-dispatcher"),
      ...
      "RoundRobinGroup" → RoundRobinGroup(List("/user/a", "/user/b")),
      "RoundRobinGroup with custom dispatcher" → RoundRobinGroup(List("/user/a", "/user/b"), routerDispatcher = "my-dispatcher"),
      "RoundRobinPool" → RoundRobinPool(25),
      "ScatterGatherFirstCompletedGroup" → ScatterGatherFirstCompletedGroup(List("/user/a", "/user/b"), routerDispatcher = "my-dispatcher", within = 3.seconds),
      "ScatterGatherFirstCompletedPool" → ScatterGatherFirstCompletedPool(25, within = 3.seconds),
      "SmallestMailboxPool" → SmallestMailboxPool(25),
      "TailChoppingGroup" → TailChoppingGroup(List("/user/a", "/user/b"), within = 3.seconds, interval = 1.second),
