 ...
 * INTERNAL API
 *
 * This contains a Scala port of the 128-bit hash function `MurmurHash3_x64_128` from Austin Appleby's original
 * `c++` code in SMHasher. It was ported from `MurmurHash3` in commons-codec,
 *
 * SMHasher public domain code with no copyrights. Commons codec is licensed under the Apache License, Version 2.0.
 */
@InternalApi private[akka] object Hash128 {
  ...
  private def hash128x64Internal(data: Array[Byte], offset: Int, length: Int, seed: Long): (Long, Long) = {
    ...
    @tailrec def switchAllLower(n: Int): Unit = {
      (n: @switch) match {
        case 15 =>
    ...
    switchAllLower(offset + length - index)
